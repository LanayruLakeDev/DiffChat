...
export class DiffDBManager {
  ...
  async initializeUserRepo(): Promise<void> {
    ...
    try {
      ...
      console.log('🔍 DIFFDB MANAGER: Ensuring repository is properly initialized...');
  ...
  async saveThread(thread: ChatThread): Promise<void> {
    ...
    try {
      ...
      if (existingFile) {
        console.log('  📄 Existing timeline file found, checking for duplicate thread...');
        ...
        if (existingThreadMatch && existingThreadMatch.length > 0) {
          console.log('  🔄 Found existing thread entry, updating instead of duplicating...');
          console.log('  🗑️ Removing old thread entries:', existingThreadMatch.length, 'entries');
  ...
  async saveMessage(message: ChatMessage, threadTitle: string): Promise<void> {
    ...
    const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;
    const filePath = `users/${this.githubUsername}/memories/timeline/${monthKey}.md`;      console.log('  📁 Message timeline file path:', filePath);
...
`;
  ...
  async loadThreads(limit: number = 50): Promise<ChatThread[]> {
    ...
    for (const filePath of sortedFiles.slice(0, 10)) { // Limit to recent files
      ...
      console.log('📚 DIFFDB MANAGER: File content length:', file.content.length);
      console.log('📚 DIFFDB MANAGER: File content preview (first 500 chars):', file.content.substring(0, 500));
      ...
      const threadMatches = Array.from(content.matchAll(/#### Thread: ([^(]*?)\s*\(([^)]+)\)/g));
      
      console.log('📚 DIFFDB MANAGER: Found', threadMatches.length, 'thread matches');
      
      for (const match of threadMatches) {
        const title = (match[1] || "").trim() || "New Chat"; // Handle empty titles
        const id = (match[2] || "").trim();
        
        console.log('📚 DIFFDB MANAGER: Processing thread match:', { title, id, match: match[0] });
        ...
        threads.push(thread);
        console.log('📄 DIFFDB: Successfully parsed thread:', { id, title, userId: this.userId, createdAt });
    ...
    console.log('✅ DIFFDB MANAGER: Final unique threads count:', uniqueThreads.length);
    console.log('✅ DIFFDB MANAGER: Loaded unique threads:', uniqueThreads.map(t => ({ id: t.id, title: t.title })));
